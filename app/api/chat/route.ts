...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    messages: await convertToModelMessages(messages.slice(-20)), // bounded context window
    temperature: 0.6,
    maxOutputTokens: 700,
  ...
  return result.toUIMessageStreamResponse({
    ...
    onFinish: async ({ messages: finalMessages }) => {
      ...
      await saveTurn({ chatId, messages: finalMessages.slice(-2) }); // the new user + assistant turn
    },
  });
}