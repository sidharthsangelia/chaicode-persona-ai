...
import { clearGuestDraft, loadGuestDraft, saveGuestDraft } from "@/lib/chat/guestDraft";
...
export function ChatView({ chatId, initialMessages, initialPersonaId }: ChatViewProps) {
  ...
  useEffect(() => {
    ...
    // Note: pendingChatId intentionally NOT changed here — see below.
  ...
  // Import into Postgres the instant sign-in is true and this chat hasn't
  // been saved yet — covers both same-tab modal sign-in AND a full reload
  // after sign-up, with zero mid-life `id` switching.
  const hasImportedRef = useRef(false);
  useEffect(() => {
    if (!isLoaded || !isSignedIn || chatId || hasStartedChat || hasImportedRef.current) return;
    if (messages.length === 0) return;

    hasImportedRef.current = true;
    (async () => {
      const result = await importGuestChatAction({ chatId: pendingChatId, personaId, messages });
      if (result?.error) {
  ...
  }, [isLoaded, isSignedIn, chatId, hasStartedChat, messages, pendingChatId, personaId, router]);
  ...
  function handleEditMessage(messageId: string, newText: string) {
    ...
    if (index === -1) return;
    setMessages([...messages.slice(0, index), { ...messages[index], parts: [{ type: "text", text: newText }] }]);
    regenerate();
  }

  return (
    <div className="flex h-dvh flex-1 flex-col bg-background">
      <ChatHeader persona={persona} personaId={personaId} onPersonaChange={handlePersonaChange} startsNewChatOnSwitch={messages.length > 0} />
      <ChatMessages
        messages={messages}
        persona={persona}
      ...
      <div className="bg-background/95 p-4 backdrop-blur supports-[backdrop-filter]:bg-background/80">
        {guestLimitReached ? <GuestLimitBanner /> : (
